...
import { createGroupAction } from '../../../../redux/actions/fetchCouriersAction';
import { useNavigate } from 'react-router-dom';
import Spinner from '../../../../components/ui/spinner';
import * as Yup from 'yup';
import { createCustomerAction } from '../../../../redux/actions/fetchCustomersAction';
import { ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import CountryFlag from '../../../supportFeatures/couriers/components/CountryFlag';
...
const CreateCustomer = () => {
    ...
    const handleAddCustomer = () => {
        ...
        dispatch(createCustomerAction(payload, navigate));

    }

  return (
    <div className='bg-[#F8F9FA] h-[93%] w-full flex flex-col px-10 py-6 overflow-y-auto pb-32'>
        <div className="text-zinc-800 text-2xl font-bold font-['Rubik']">Customers</div>
        <div className="w-[266px] h-5 justify-start items-center gap-[9px] inline-flex mt-4">
            <div className="text-gray-400 text-sm font-normal font-['Rubik'] leading-tight">Customers</div>
            <img src={RightArrow} alt='SVG' className='w-3 h-3' />
            <div className="text-red-800 text-sm font-normal font-['Rubik'] leading-tight">Add Customer</div>
        </div> 
        <div className="mt-6 w-full h-[331px] p-6 bg-white rounded-[10px] flex-col justify-start items-start inline-flex">
                    ...
                    <Listbox value={data.country} onChange={(item) => setData({ ...data, country: item })}>
                        ...
                        <Listbox.Button className="self-stretch w-full h-12 px-4 py-[13px] bg-white placeholder:text-gray-300 text-sm font-normal font-['Rubik'] leading-tight text-zinc-800 rounded-xl border border-zinc-200 justify-start items-center gap-2.5 inline-flex">
                            <div className='flex flex-row gap-2 items-center'>
                                {data.country.id > 0 && <CountryFlag countryName={data.country.name} />}
            ...
            <button disabled={!validated} onClick={handleAddCustomer} className={`w-[260px] mt-8 h-[50px] px-[60px] py-[15px] ${validated ? 'bg-red-800' : 'bg-gray-100'} rounded-xl justify-center items-center gap-2.5 inline-flex text-center  ${validated ? 'text-white' : 'text-gray-400'} text-base font-normal font-['Rubik'] leading-tight`}>
                {
                    loading ? <Spinner className={'fill-white'} /> : 'Add Cutomer'
                } 
            </button>
        </div>
        <ToastContainer />
