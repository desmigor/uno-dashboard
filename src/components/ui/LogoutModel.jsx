  ...
  return (
    <div className='w-[275px] px-3 py-[14px] h-[120px] absolute z-10 top-[54px] rounded-lg right-[28px] bg-white shadow-xl flex flex-col'>
      <div className="w-[189px] h-[38px] justify-start items-center gap-3 inline-flex">
        <img className="w-8 h-8 rounded-full object-cover bg-red-50" src={userInfo?.profile_photo_link ? userInfo?.profile_photo_link : PlaceHolderImage } />
        <div className="flex-col justify-center items-start gap-0.5 inline-flex">
            <div className="text-black text-sm font-normal font-rubik leading-tight">{userInfo?.full_name.slice(0, 1).toUpperCase() + userInfo?.full_name.slice(1) }</div>
            <div className="text-gray-400 text-xs font-normal font-rubik leading-none">{userInfo?.email}</div>
        </div>
      </div>
